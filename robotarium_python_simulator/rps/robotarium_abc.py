...
class RobotariumABC(ABC):
    ...
    def __init__(self, number_of_robots=-1, show_figure=True, sim_in_real_time=True, initial_conditions=np.array([]), number_of_shepherds = 1):
        ...
        self.colors = ['b' for i in range(number_of_robots-number_of_shepherds)] + ['r' for i in range(number_of_shepherds)]
        ...
        if(self.show_figure):
            ...
            for i in range(number_of_robots):
                p = patches.RegularPolygon(self.poses[:2, i], 4, math.sqrt(2)*self.robot_radius, self.poses[2,i]+math.pi/4, facecolor=self.colors[i], edgecolor = 'k')
